...
\begin{itemize}
    ...
    \item[\textbf{齐次性：}] 假设\[
            x_0 \ce{H2S} + y_0 \ce{SO2} \longrightarrow z_0 \ce{S} + t_0 \ce{H2O}
        \]
    ...
    \item[\textbf{可加性：}] 随便的两个已配平的化学方程式：\[
            x_1 \ce{H2S} + y_1 \ce{SO2} \longrightarrow z_1 \ce{S} + t_1 \ce{H2O}
        \]
        和
        \[
            x_2 \ce{H2S} + y_2 \ce{SO2} \longrightarrow z_2 \ce{S} + t_2 \ce{H2O}
        \]
        它们的和
        \[
...
我们求解一下前面的线性方程组：
\begin{alignat}{4}
    x  & + y &  & + z^{\prime} &  &               &  & = 0 \label{eq:1} \\
    2x &     &  &              &  & + 2t^{\prime} &  & = 0 \label{eq:2} \\
    & 2y  &  &              &  & + t^{\prime}  &  & = 0 \label{eq:3}
\end{alignat}
\begin{figure}[H]
    \centering
...
仔细观摩你见过的多解的化学方程式：
\begin{alignat*}{5}
    &         & \ce{CH4} &+ \ce{O2}  &&\dash \ce{CO2} +&& \ce{CO}  &&+ \ce{H2O} \\
    &=\lambda(& \ce{CH4} &+ 2\ce{O2} &&\longrightarrow \ce{CO2} &&           &&+ \ce{2H2O}) \\
    &+\mu(    & \ce{2CH4}&+ 3\ce{O2} &&\longrightarrow            &&2\ce{CO} &&+ 4\ce{H2O})
\end{alignat*}
\begin{alignat*}{5}
    &         & \ce{Cl-} &+ \ce{ClO3^-}  &&\dash \ce{Cl2} +&& \ce{ClO2}  &&+ \ce{H2O} \\
    &=\lambda(& \ce{5Cl-} &+ \ce{ClO3^-} &&\longrightarrow \ce{3Cl2} &&           &&+ \ce{3H2O}) \\
    &+\mu(    & \ce{Cl-}&+ \ce{5ClO3^-} &&\longrightarrow           &&\ce{6ClO2} &&+ \ce{3H2O})
\end{alignat*}
...
\hfill
\begin{minipage}[c]{0.55\textwidth}
    对于一个配平了的化学方程式，记元素\(e\)的化合价前后为\(v(e)\)与\(v(e^{\prime})\)，
    系数为\(k_{e}\)。\\[1em]
...
\begin{example}[基于电子的半反应法]
    ...
    \end{align}
    \(\text{\Cref{eq:half-reaction-1}}\times 2 - \text{\Cref{eq:half-reaction-2}}\times 5\)：
    \[
...
\begin{example}[基于氧原子的半反应法]
    ...
    可以分成两个半反应：
    \begin{align}
        &\ce{2FeS2 + H2O + 15O -> Fe2(SO4)3 + H2SO4} \label{eq:half-reaction-3} \\
        &\ce{2HNO3 -> 2NO ^ + H2O+ + 3O} \label{eq:half-reaction-4}
    \end{align}
    \(\text{\Cref{eq:half-reaction-3}}+\text{\Cref{eq:half-reaction-4}}\times 5\)：
    \[
