...
仔细观摩你见过的多解的化学方程式：
\begin{alignat*}{5}
    &         & \ce{CH4} &+ \ce{O2}  &&- \ce{CO2} +&& \ce{CO}  &&+ \ce{H2O} \\
    &=\lambda(& \ce{CH4} &+ 2\ce{O2} &&\to \ce{CO2} &&           &&+ \ce{2H2O}) \\
    &+\mu(    & \ce{2CH4}&+ 3\ce{O2} &&\to            &&2\ce{CO} &&+ 4\ce{H2O})
\end{alignat*}
\begin{alignat*}{5}
    &         & \ce{Cl-} &+ \ce{ClO3^-}  &&- \ce{Cl2} +&& \ce{ClO2}  &&+ \ce{H2O} \\
    &=\lambda(& \ce{5Cl-} &+ \ce{ClO3^-} &&\to \ce{3Cl2} &&           &&+ \ce{3H2O}) \\
    &+\mu(    & \ce{Cl-}&+ \ce{5ClO3^-} &&\to           &&\ce{6ClO2} &&+ \ce{3H2O})
\end{alignat*}
...
化学方程式解的结构就是齐次方程组的解空间结构。若存在\(\mathrm{u}, \mathrm{v}\)满足\(A\x = \b\)，
那显然\(\lambda \mathrm{u} + \mu \mathrm{v}\) 也满足 \(A\x = \b\)，
其中 \(\lambda, \mu\) 为任意常数，
总可以选取\(\lambda, \mu\) 使得\(\lambda \mathrm{u} + \mu \mathrm{v}\) 中的某个物种的系数为零，
从而得到一个新的化学方程式解。
...
你有没有想过，你初学时怎么就能一眼看出来\(\ce{2H2O \to 2 H2 ^ + O2 ^}\)怎么配平？你有没有想过，
...
\begin{example}[观察法配平电极反应]
    以酸性条件下\(\ce{C2H5OH} - \ce{O2}\) 的原电池反应为例，阳极发生乙醇的氧化：
    \begin{align*}
        \ce{C2H5OH} &\ce{- CO2} \\
        \intertext{首先配平碳元素：}
        \ce{C2H5OH} &\ce{- 2 CO2} \\
        \intertext{其次用水配平氧元素：}
        \ce{C2H5OH + 3 H2O} &\ce{- 2 CO2} \\
        \intertext{用氢离子配平氢元素：}
        \ce{C2H5OH + 3 H2O} &\ce{- 2 CO2 + 12 H+} \\
        \intertext{最后用电子配平电荷：}
        \ce{C2H5OH + 3 H2O} &\ce{-> 2 CO2 + 12 H+ + 12 e^-}
    \end{align*}
...
% * This requires double compilation
\[
    \vspace{5mm}
...
并且便于配平的分配给元素的数字}\UseVerb{smile}。\footnote{感谢南开大学赵庆老师对这部分的热情解答！\UseVerb{smile}}
...
\section{高手过招}
在最初提出化学方程式配平的线性代数方法的论文里，作者自豪的宣称：
\begin{multline*}
...
\subsection{正仿射幺半群}
机器确实分不清反应物和生成物。如果方程式只有一组解，那很容易区分出反应物和生成物；但如果方程式有多组解，就可能出现：
\begin{align*}
    &\placeholder{=}\ce{MnO4^- + H2O2 + H^+ - Mn^2+ + O2 + H2O} \\
...
2024年，北京化工大学在arXiv上发布了一篇论文\cite{BalancingChemicalEquations2024}，
指出为避免负系数，化学方程式配平实际的数学结构，应该是正仿射幺半群。而方程式的解，就是仿射幺半群所对应的Hilbert基。
可以说配平配出了新的数学高度。
\subsection{化学方程式解析}
